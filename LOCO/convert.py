...
def convert_pth_to_onnx(pth_path, onnx_path, seq_length=100, input_size=6):
    ...
    # Export to ONNX with dynamic batch axis, using legacy exporter to avoid dynamo issues with LSTM
    torch.onnx.export(
        model,
        dummy_input,
        onnx_path,
        ...
        dynamo=False  # Force legacy TorchScript exporter to bypass dynamo/FX tracing issues in LSTM
    )
...
def main():
    ...
    parser.add_argument('--start_trial', type=int, default=1, help='Starting trial (default: 1)')
    parser.add_argument('--end_trial', type=int, default=33, help='Ending trial (default: 33)')
    ...
    for trial in range(start_trial, end_trial + 1):
        pth_file = os.path.join(pth_dir, f'model_without_trial_{trial}.pth')
        onnx_file = os.path.join(pth_dir, f'model_without_trial_{trial}.onnx')
        ...
        if os.path.exists(pth_file):
            ...
            print(f'Warning: {pth_file} not found, skipping trial {trial}')
